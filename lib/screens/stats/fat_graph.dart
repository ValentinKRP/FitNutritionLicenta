...
class FatGraph extends StatefulWidget {
  const FatGraph({Key? key}) : super(key: key);
...
class _FatGraphState extends State<FatGraph> {
  ...
  Widget build(BuildContext context) {
    ...
    for (double i = 0; i < 7; i++) {
      ...
      var fatsPerMeal = meals.map((meal) => meal.calculateFat()).toList();
      double totalFats =
          fatsPerMeal.reduce((value, element) => value + element);

      fatsSpot.add(FlSpot(6 - i, totalFats / 100));

      currDatetime = currDatetime.subtract(const Duration(days: 1));
      currDate = currDatetime.day.toString() +
          "/" +
  ...
  LineChartData mainData(caloriesSpots) {
